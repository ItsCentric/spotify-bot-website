...
import { convertDuration, generateArtistListJSX, toPascalCase } from '../lib/utils';
import SpotifyLink from './SpotifyLink';

export default function TopTenItem(props: { item: SpotifyApi.TrackObjectFull | SpotifyApi.ArtistObjectFull }) {
    ...
    const itemAltText = trackOrArtist ? 'track cover for ' + item.name : 'artist image for ' + item.name;
    const itemSubText = trackOrArtist ? artists : `Top ${item.popularity}%`;
    const secondColumnText = trackOrArtist ? 'Appears on:' : 'Genres:';
    const secondColumnData = trackOrArtist ? <SpotifyLink link={item.album.external_urls.spotify} bold='medium'>{item.album.name}</SpotifyLink> : <div className='font-medium truncate'>{genres.join(', ')}</div>;
    const thirdColumnText = trackOrArtist ? convertDuration(new Date(item.duration_ms)) : '10 Liked Songs';
            ...
            <div className='flex'>
                ...
                <div className='max-w-full flex flex-col justify-center'>
                    <div className='text-lg truncate'>
                        <SpotifyLink link={item.external_urls.spotify} bold='medium'>{item.name}</SpotifyLink>
                    </div>
