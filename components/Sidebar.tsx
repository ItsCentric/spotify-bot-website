...
export default function Sidebar(props: { setStatType: Function }) {
    ...
    return (
        <div className={'p-4 bg-[#2F2538] drop-shadow-2xl rounded-lg max-h-fit flex flex-col mb-4 lg:m-0'}>
            <div className='flex justify-between lg:justify-start'>
                <div>
                    ...
                    <div className='ml-3 inline-block align-middle max-w-[100px]'>
                        <a href={spotifyData.userInfo.external_urls.spotify} className='text-xl font-medium hover:text-green-light transition-colors'>{session ? session.user.name : ''}</a>
                        <p className='text-white/80'>{userInfo.followers.total + ' Followers'}</p>
                    </div>
                </div>
                <button className='lg:hidden' onClick={() => setOpen((previousOpen) => { return !previousOpen })}>
                    <BiMenu className='h-8 w-8' />
                </button>
            </div>
            <div className={'lg:flex-1 lg:flex lg:flex-col justify-center my-4 lg:m-0 ' + (open ? 'block' : 'hidden')}>
                <ul className='flex flex-col lg:items-center divide-y-2 divide-white/80'>
                    ...
                    <li className='pt-2 text-red-400 group relative'>
                        <div className='flex items-center cursor-default'>
                            ...
                            <p>Music Player</p>
                            <Tooltip position='top' alignment='left'>This feature is a work in progress!</Tooltip>
                        </div>
                    </li>
                </ul>
            </div>
