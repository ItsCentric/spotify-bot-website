...
export default function SpotifyStatCard(props: {
  data: SpotifyApi.TrackObjectFull | SpotifyApi.ArtistObjectFull;
}) {
  ...
  if ('album' in props.data) {
    ...
    return (
      <div
        className={
          'bg-blackRaspberry-600 lg:space-y-2 p-4 rounded-md grid grid-cols-2 gap-2 lg:gap-0 lg:block lg:grid-cols-none min-w-0'
        }>
        <div className='relative'>
          <div className='absolute right-0 bottom-0 bg-white rounded-tl-lg px-1 py-0.5'>
            <p className='text-blackRaspberry-900'>
              {convertDuration(new Date(props.data.duration_ms))}
            </p>
          </div>
          <Image
            ...
            width={640}
            height={640}
            className='aspect-square object-cover'></Image>
        </div>
    ...
    return (
      <div className='bg-white flex flex-col gap-2 p-4 rounded-md flex-1 min-w-0'>
        ...
        <div>
          <p className='text-blackRaspberry font-medium text-2xl truncate'>
            <a
              href={props.data.external_urls.spotify}
          ...
          <p className='text-blackRaspberry/75 font-medium text-lg capitalize truncate'>
            {genres.toString()}
          </p>
        </div>
        <div className='divide-y-2 border-y-2'>
          <p className='text-blackRaspberry text-lg'>
            <span className='font-medium'>Followers:</span>{' '}
            {numberFormatter.format(props.data.followers.total)}
          </p>
          <p className='text-blackRaspberry text-lg'>
            <span className='font-medium'>Popularity:</span> {props.data.popularity}
          </p>
        </div>
      </div>
    );
