...
export default function TopTen(props: {
  ...
  timeRangeState: {
    ...
    changeValue: (value: 'short_term' | 'medium_term' | 'long_term') => void;
  };
}) {
  ...
  const itemsJsx = items.map((item: SpotifyApi.TrackObjectFull | SpotifyApi.ArtistObjectFull) => {
    return <TopTenItem item={item} key={item.id} />;
  });
      ...
      <div className='flex justify-between items-center'>
        ...
        <div>
          <ul className='divide-x-2 divide-white/80 text-white/75 text-sm'>
            <button
              className={
                'pr-2 hover:text-white transition-colors ' +
                (timeRange.value === 'short_term' ? 'text-white' : '')
              }
              onClick={() => timeRange.changeValue('short_term')}>
              Last 4 Weeks
            </button>
            <button
              className={
                'px-2 hover:text-white transition-colors ' +
                (timeRange.value === 'medium_term' ? 'text-white' : '')
              }
              onClick={() => timeRange.changeValue('medium_term')}>
              Last 6 Months
            </button>
            <button
              className={
                'pl-2 hover:text-white transition-colors ' +
                (timeRange.value === 'long_term' ? 'text-white' : '')
              }
              onClick={() => timeRange.changeValue('long_term')}>
              All Time
            </button>
          </ul>
        </div>
